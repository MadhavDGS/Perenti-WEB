    ...
    <aside className="right-panel">
      ...
      <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)', marginBottom: 32 }}>
        {[
          ...
          <div key={s.label} className="stat-card" style={{ padding: 'var(--space-3)', background: 'var(--bg-card)', border: 'none', borderRadius: 'var(--radius-md)', textAlign: 'center' }}>
            ...
            <div className="stat-label" style={{ fontSize: '0.6875rem', color: 'var(--text-secondary)', fontWeight: 600, textTransform: 'uppercase', tracking: '0.05em' }}>{s.label}</div>
          </div>
        ))}
      </div>

      {/* Who to Connect */}
      <h3 style={{ fontSize: '1.15rem', fontWeight: 500, color: 'var(--text-primary)', marginBottom: 'var(--space-4)', fontFamily: 'var(--font-display)' }}>Who to Connect</h3>
        ...
        {suggestions.map((m, i) => (
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: '0.8125rem', fontWeight: 700, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.name}</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.profession}</div>
            </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          <span style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-primary)' }}>Complete Your Profile</span>
        </div>
        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 12 }}>
          Get 3× more connection requests with a complete profile.
        </p>
        <Link to="/profile/me" className="btn btn-primary btn-sm" style={{ display: 'flex', justifyContent: 'center', width: '100%', textDecoration: 'none' }}>
          <CheckCircle size={14} />
          Complete Now
        </Link>
      </div>
